...
pg_wait_running "$T" 1800 || fail TARGET_NOT_RUNNING "$T"
sts_wait_ready "$T" 1800 || fail TARGET_REPLICAS_NOT_READY "$T"

# ---- 5. hand the new instance to Argo CD (cross-cluster restores)
if [[ "$FLEET_ADDED" == "true" ]]; then
  if sync_instance_app "$DST_C" "$T" "$TIMEOUT"; then
    NOTES+=("Application tpg-${DST_C}-${T} synced: the instance is managed by Argo CD")
  else
    NOTES+=("Application tpg-${DST_C}-${T} did not sync: check it in Argo CD")
  fi
